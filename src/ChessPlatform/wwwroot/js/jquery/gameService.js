...
var GameService = function (webSocketService, gameModel) {
    ...
    var onMouseoverSquare = function (square, piece) {
        ...
        if ((self.chess.turn() === 'w' && !self.gameModel.currentUserIsPlayer1) || (self.chess.turn() === 'b' && !self.gameModel.currentUserIsPlayer2) || moves.length === 0) {
            return;
        }
    ...
    this.getComputerMove = function () {
        $.get('/game/getbestmove?fen=' + self.chess.fen(), function (response) {
    ...
    this.postMove = function (move, callback) {
        ...
        $.post('/game/makeMove', makeMoveModel, function (response) {
    ...
    this.onEnd = function (winGame) {
        ...
        if (self.gameModel.currentUserIsPlayer) {
            if (!self.gameModel.gameInfo.withComputer) {
                $notificationContainer.find('.notification-message').html(winGame ? "You're the best!" : "Maybe next time.");
            }
            else {
                $notificationContainer.find('.notification-message').html(self.chess.turn() == 'b' ? "You're the best!" : "Maybe next time.");
            }
        }
        else {
            if (!self.gameModel.gameInfo.withComputer) {
                $notificationContainer.find('.notification-message').html((self.chess.turn() == 'w' ? self.gameModel.gameInfo.player2 : self.gameModel.gameInfo.player1) + " has win this game!");
            }
            else {
                $notificationContainer.find('.notification-message').html((self.chess.turn() == 'w' ? "Computer" : self.gameModel.gameInfo.player1) + " has win this game!");
            }
        }
