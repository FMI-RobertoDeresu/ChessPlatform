...
(function() {
    ...
    function gameController($scope, $window, $uibModal, NgTableParams, gameService, userService, notificationService,
        DEFAULT_URL) {
        ...
        $scope.joinGame = function(id) {
            $window.location.href = `${DEFAULT_URL}game?id=${id}`;
        };
                    ...
                    if (response.gameInfo.started) {
                        ...
                        if (response.gameInfo.requirePassword === false || response.currentUserIsPlayer) {
                            ...
                            var modalInstance = $uibModal.open({
                                animation: true,
                                templateUrl: `${DEFAULT_URL}views/game/playModal.html`,
        ...
        $scope.updateProfilePhotoModal = function() {
            var modalInstance = $uibModal.open({
                animation: true,
                templateUrl: `${DEFAULT_URL}views/user/updateProfilePhotoModal.html`,
        ...
        $scope.gameFilter = function() {
            ...
            var filteredGames = $scope.games.filter(function(item) {
                ...
                if ($scope.filter.player != null &&
                    !includesLower(item.gameInfo.player1.profile.nickname, $scope.filter.player) &&
                    !includesLower(item.gameInfo.player2.profile.nickname, $scope.filter.player)) {
                    return false;
                }

                if ($scope.filter.points != null &&
                ((item.gameInfo.minimumNumberOfPoints != null &&
                        item.gameInfo.minimumNumberOfPoints > $scope.filter.points) ||
                    (item.gameInfo.maximumNumberOfPoints != null &&
                        item.gameInfo.maximumNumberOfPoints < $scope.filter.points))) {
                    return false;
                }
